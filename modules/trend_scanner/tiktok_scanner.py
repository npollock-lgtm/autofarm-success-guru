"""
TikTok trend scanner for AutoFarm Zero — Success Guru Network v6.0.

Scans for TikTok-relevant viral trends using two indirect data sources
(direct TikTok scraping is blocked from cloud IPs):

1. **Primary:** Google Trends via pytrends — queries brand keywords
   contextualized for TikTok (e.g. "dark psychology tiktok") to discover
   what people are searching for related to TikTok content.

2. **Fallback:** Google News RSS — searches for articles about TikTok
   trends in each brand's niche (e.g. "tiktok trending psychology").

Each brand has ``tiktok_keywords`` defined in ``brands.json`` under
``trend_sources``.
"""
...
class TikTokScanner(BaseScanner):
    """
    Scans for TikTok viral trend topics.

    Uses Google Trends (primary) and Google News RSS (fallback) with
    TikTok-contextualized keywords to discover trending topics on
    TikTok relevant to each brand's niche.
    ...
    def __init__(self) -> None:
        """
        Initializes the TikTok scanner.
        ...
        try:
            ...
            logger.info("pytrends_not_available_for_tiktok")

        self.session = requests.Session()
        self.session.headers.update({
            "User-Agent": "AutoFarm/6.0 TikTok Scanner",
            "Accept": "application/rss+xml, application/xml, text/xml",
        })

    def scan(self, brand_id: str, brand_config: dict) -> list[TrendItem]:
        """
        Scans for TikTok trends relevant to the brand.
        ...
        Returns:
            List of TrendItem objects from TikTok-related sources.

        Side effects:
            Makes HTTP requests to Google Trends and/or Google News RSS.
        """
        trend_sources = brand_config.get("trend_sources", {})
        tiktok_keywords = trend_sources.get("tiktok_keywords", [])

        if not tiktok_keywords:
            logger.info("no_tiktok_keywords", brand_id=brand_id)
            return []

        all_trends = []
        brand_keywords = self._extract_all_keywords(brand_config)

        # Primary: Google Trends with TikTok context
        if self._pytrends_available:
            gt_trends = self._fetch_tiktok_google_trends(
                tiktok_keywords, brand_keywords, brand_id
            )
            all_trends.extend(gt_trends)

        # Fallback / supplement: Google News RSS
        news_trends = self._fetch_tiktok_news_trends(
            tiktok_keywords, brand_keywords, brand_id
        )
    ...
    def _fetch_tiktok_google_trends(
        self,
        keywords: list[str],
        ...
        """
        Fetches TikTok-contextualized search trends via pytrends.

        Parameters:
            keywords: TikTok-specific seed keywords.
        ...
        for i in range(0, len(keywords), max_per_request):
            ...
            try:
                ...
                logger.warning(
                    "tiktok_google_trends_batch_error",
                    brand_id=brand_id,
                    keywords=batch,
                    error=str(e),
                )
    ...
    def _fetch_trends_batch(
        ...
        """
        Fetches a single batch of TikTok search trends via pytrends.
                    ...
                    for _, row in rising.iterrows():
                        ...
                        trends.append(
                            TrendItem(
                                topic=query_text,
                                source="tiktok",
                                source_detail=f"google_trends:tiktok:{keyword}",
                                raw_data=raw_data,
                    ...
                    for _, row in top.head(10).iterrows():
                        ...
                        trends.append(
                            TrendItem(
                                topic=query_text,
                                source="tiktok",
                                source_detail=f"google_trends:tiktok_top:{keyword}",
                                raw_data=raw_data,
                                relevance_score=relevance,
                    ...
                    for _, row in rising.iterrows():
                        ...
                        trends.append(
                            TrendItem(
                                topic=topic_title,
                                source="tiktok",
                                source_detail=f"google_trends:tiktok_topic:{keyword}",
                                raw_data=raw_data,
                                relevance_score=relevance,
                                engagement=int(value) if value else 0,
                            )
    ...
    def _fetch_tiktok_news_trends(
        ...
        """
        Fetches TikTok-related news articles via Google News RSS.

        Searches for articles about TikTok trends in the brand's niche.

        Parameters:
            keywords: Brand keywords to search with TikTok context.
            brand_keywords: All brand keywords for relevance scoring.
            brand_id: Brand identifier for logging.
        ...
        # Construct TikTok-focused search queries from brand keywords
        # Use the raw niche keywords (strip "tiktok" suffix if present)
        search_terms = []
        for kw in keywords[:5]:
            # Build search like "tiktok trending dark psychology"
            clean = kw.replace(" tiktok", "").replace("tiktok ", "").strip()
            search_terms.append(f"tiktok trending {clean}")
            ...
            for item in channel.findall("item")[:5]:
                ...
                trends.append(
                    TrendItem(
                        topic=clean_title,
                        source="tiktok",
                        source_detail=f"news:tiktok:{search_query}",
                        raw_data=raw_data,
                        relevance_score=relevance,
                        url=link,
                    )
    ...
    def _extract_all_keywords(self, brand_config: dict) -> list[str]:
        ...
        keywords.extend(trend_sources.get("tiktok_keywords", []))
