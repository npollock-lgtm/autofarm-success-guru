"""
Instagram trend scanner for AutoFarm Zero — Success Guru Network v6.0.

Scans for Instagram-relevant viral trends using two indirect data sources
(direct Instagram scraping is blocked from cloud IPs due to TLS
fingerprinting and IP quality detection):

1. **Primary:** Google Trends via pytrends — queries brand keywords
   contextualized for Instagram (e.g. "dark psychology reels",
   "stoic quotes instagram") to discover what people are searching for
   related to Instagram content.

2. **Fallback:** Google News RSS — searches for articles about Instagram
   trends in each brand's niche.

Each brand has ``instagram_keywords`` defined in ``brands.json`` under
``trend_sources``.
"""
...
class InstagramScanner(BaseScanner):
    """
    Scans for Instagram viral trend topics.

    Uses Google Trends (primary) and Google News RSS (fallback) with
    Instagram-contextualized keywords to discover trending topics on
    Instagram relevant to each brand's niche.
    ...
    def __init__(self) -> None:
        """
        Initializes the Instagram scanner.
        ...
        try:
            ...
            logger.info("pytrends_not_available_for_instagram")

        self.session = requests.Session()
        self.session.headers.update({
            "User-Agent": "AutoFarm/6.0 Instagram Scanner",
            "Accept": "application/rss+xml, application/xml, text/xml",
        })

    def scan(self, brand_id: str, brand_config: dict) -> list[TrendItem]:
        """
        Scans for Instagram trends relevant to the brand.
        ...
        Returns:
            List of TrendItem objects from Instagram-related sources.

        Side effects:
            Makes HTTP requests to Google Trends and/or Google News RSS.
        """
        trend_sources = brand_config.get("trend_sources", {})
        ig_keywords = trend_sources.get("instagram_keywords", [])

        if not ig_keywords:
            logger.info("no_instagram_keywords", brand_id=brand_id)
            return []

        all_trends = []
        brand_keywords = self._extract_all_keywords(brand_config)

        # Primary: Google Trends with Instagram context
        if self._pytrends_available:
            gt_trends = self._fetch_instagram_google_trends(
                ig_keywords, brand_keywords, brand_id
            )
            all_trends.extend(gt_trends)

        # Fallback / supplement: Google News RSS
        news_trends = self._fetch_instagram_news_trends(
            ig_keywords, brand_keywords, brand_id
        )
    ...
    def _fetch_instagram_google_trends(
        self,
        keywords: list[str],
        ...
        """
        Fetches Instagram-contextualized search trends via pytrends.

        Parameters:
            keywords: Instagram-specific seed keywords.
        ...
        for i in range(0, len(keywords), max_per_request):
            ...
            try:
                ...
                logger.warning(
                    "instagram_google_trends_batch_error",
                    brand_id=brand_id,
                    keywords=batch,
                    error=str(e),
                )
    ...
    def _fetch_trends_batch(
        ...
        """
        Fetches a single batch of Instagram search trends via pytrends.
        ...
        try:
                    ...
                    for _, row in rising.iterrows():
                        ...
                        trends.append(
                            TrendItem(
                                topic=query_text,
                                source="instagram",
                                source_detail=f"google_trends:instagram:{keyword}",
                                raw_data=raw_data,
                    ...
                    for _, row in top.head(10).iterrows():
                        ...
                        trends.append(
                            TrendItem(
                                topic=query_text,
                                source="instagram",
                                source_detail=f"google_trends:instagram_top:{keyword}",
                                raw_data=raw_data,
                                relevance_score=relevance,
            ...
            logger.warning(
                "instagram_related_queries_failed",
                keywords=keywords,
                error=str(e),
            )
        ...
        try:
                    ...
                    for _, row in rising.iterrows():
                        ...
                        trends.append(
                            TrendItem(
                                topic=topic_title,
                                source="instagram",
                                source_detail=f"google_trends:instagram_topic:{keyword}",
                                raw_data=raw_data,
                                relevance_score=relevance,
                                engagement=int(value) if value else 0,
                            )
            ...
            logger.warning(
                "instagram_related_topics_failed",
                keywords=keywords,
                error=str(e),
            )
    ...
    def _fetch_instagram_news_trends(
        self,
        keywords: list[str],
        brand_keywords: list[str],
        brand_id: str,
        ...
        """
        Fetches Instagram-related news articles via Google News RSS.

        Searches for articles about Instagram trends in the brand's niche.

        Parameters:
            keywords: Brand keywords to search with Instagram context.
            brand_keywords: All brand keywords for relevance scoring.
            brand_id: Brand identifier for logging.
        ...
        # Build Instagram-focused search queries
        search_terms = []
        for kw in keywords[:5]:
            clean = (
                kw.replace(" instagram", "")
                .replace("instagram ", "")
                .replace(" reels", "")
                .replace("reels ", "")
                .strip()
            )
            search_terms.append(f"instagram trending {clean}")
            ...
            for item in channel.findall("item")[:5]:
                ...
                trends.append(
                    TrendItem(
                        topic=clean_title,
                        source="instagram",
                        source_detail=f"news:instagram:{search_query}",
                        raw_data=raw_data,
                        relevance_score=relevance,
                        url=link,
                    )
    ...
    def _extract_all_keywords(self, brand_config: dict) -> list[str]:
        ...
        keywords.extend(trend_sources.get("instagram_keywords", []))
